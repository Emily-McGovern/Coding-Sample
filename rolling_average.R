...
roll.mp.case<-mp.data %>% 
  ...
  group_by(Date_confirmation) %>% #group by day and count cases
  count(Status) %>% 
  ungroup() %>% 
  complete(Date_confirmation = seq(min(Date_confirmation, na.rm = T),max(Date_confirmation, na.rm = T),1), 
  ...
  dplyr::mutate(roll = zoo::rollmean(n, k = 7, fill = NA, align = "right"),
                ...
                weekday= ifelse(day %in% c(1,7), "Weekend", "Weekday")) #calculate rolling average - average for day is calaculate based on cases for previous 7 days
