...
roll.mp.case <- mp.data %>%
  ...
  dplyr::count(Status) %>%
  dplyr::ungroup() %>%
  tidyr::complete(Date_confirmation = seq(
    min(Date_confirmation, na.rm = T),
    max(Date_confirmation, na.rm = T),
    1
  ),
  fill = list(n = 0)) %>% #add missing dates
  dplyr::mutate(
    roll = zoo::rollmean(n, k = 7, fill = NA, align = "right"),
    day = lubridate::wday(Date_confirmation),
    weekday = base::ifelse(day %in% c(1, 7), "Weekend", "Weekday")
  ) #calculate rolling average - average for day is calaculate based on cases for previous 7 days
...
p <- p +
  ggplot2::labs(
    title = "Daily Confirmed Monkey Pox Cases with moving 7-day average",
    subtitle = paste0(
      format(min(roll.mp.case$Date_confirmation), "%b %d %Y"),
      " to ",
